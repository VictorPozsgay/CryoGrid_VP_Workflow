...
%   and trend consistency (30 pts)
%
% INPUTS:
%   - stats  : structure containing RMSE, MAE, bias, R, mean difference
%   - trends : structure containing slope_obs, slope_pred, Spearman rho, directional agreement
%
% OUTPUTS:
...
%   - details : structure with detailed scoring components
    ...
    % RMSE (10 pts)
    rmse_max = 5;  % max tolerance (adjustable)
    score_rmse = max(0, 10 * (1 - stats.RMSE / rmse_max));

    % MAE (20 pts)
    mae_max = 4;  % max tolerance
    score_mae = max(0, 20 * (1 - stats.MAE / mae_max));

    % Bias (20 pts)
    bias_max = 3;  % tolerated absolute value
    score_bias = max(0, 20 * (1 - abs(stats.Bias) / bias_max));

    % R² (20 pts)
    score_r2 = max(0, 20 * stats.R2);  % R between 0 and 1

    % Mean temperature difference (10 pts)
    diff_max = 3;  % tolerated °C difference
    score_mean = max(0, 10 * (1 - abs(stats.Mean_Diff) / diff_max));

    %% --- Part 2: Trend consistency evaluation (30 points) ---

    % Slope difference (10 pts)
    slope_diff = abs(trends.Slope_Obs - trends.Slope_Pred);
    score_slope = max(0, 10 * (1 - slope_diff / 0.05));  % beyond 0.05, score = 0

    % Spearman rho (10 pts)
    score_rho = max(0, 10 * (trends.Rho_Spearman)^2);

    % Directional agreement (10 pts)
    score_agreement = max(0, 10 * (trends.Local_Directional_Agreement_Percent - 60) / 40);
    ...
    % Also include raw stats
    details.RMSE               = stats.RMSE;
    details.MAE                = stats.MAE;
    details.Bias               = stats.Bias;
    details.R2                 = stats.R2;
    ...
    details.Directional_Agreement = trends.Local_Directional_Agreement_Percent;
end
