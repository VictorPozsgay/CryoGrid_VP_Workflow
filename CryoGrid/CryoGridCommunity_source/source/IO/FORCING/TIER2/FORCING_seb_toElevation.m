%========================================================================
% CryoGrid FORCING class FORCING_seb_toElevation
%
% model forcing for GROUND classes computing the surface energy 
% balance (keyword "seb"). 
% 
% The data is obtained by linearly interpolating data in elevation
% (from SAFRAN reanalysis) to the elevation of the target location, and
% selecting the massif (no horizontal interpolation yet) to which the
% target location belongs. The input data is provided in a mat-file with
% the following properties
% 
% - the structure is called 'FORCING'
% - it is a 1xNmassifs (number of massifs) array with fields:
%       - name       % name of the massif, e.g. 'Aravis'
%       - massif_num % massif number, between 1 and 23 for the Alps
%       - lon        % longitude (WGS84) of the massif centroid
%       - lat        % latitude (WGS84) of the massif centroid
%       - data       % data structure
%           - t_span: [Ntx1]
%           - S_TOA:  [Ntx1]
%           - z:      [1xNz]
%           - Tair, Lin, Sin, q, wind, p, rainfall, snowfall: [NtxNz]
%       - polygon    % shapefile structure
%           - X:   X coordinates of the SAFRAN polygons (Lambert-93)
%           - Y:   Y coordinates of the SAFRAN polygons (Lambert-93)
%           - Lon: Lon coordinates of the SAFRAN polygons (WGS84)
%           - Lat: Lat coordinates of the SAFRAN polygons (WGS84)
% 
...
classdef FORCING_seb_toElevation < FORCING_base
    ...
    methods
        function forcing = provide_PARA(forcing)
            ...
            forcing.PARA.data_product = []; %SAFRAN (default when left empty)
        ...
        function forcing = finalize_init(forcing, tile)
            ...
            if isempty(forcing.PARA.data_product) || sum(isnan(forcing.PARA.data_product))>0
                forcing.PARA.data_product = 'SAFRAN';
            end
            ...
            if strcmp(forcing.PARA.data_product, 'SAFRAN')
                proc_functions = {'read_mat_SAFRAN'; 'set_start_end_time'; 'interpolate_elevation_massif_SAFRAN'; 'assign_lb_heatflux_airT_height'; 'scale_precip'; 'check_and_correct'};
                % proc_functions = {'read_mat_SAFRAN'; 'set_start_end_time'; 'interpolate_elevation_massif_SAFRAN'};
            end
