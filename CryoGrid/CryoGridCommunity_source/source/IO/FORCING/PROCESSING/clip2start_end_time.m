    ...
    methods
        ...
        function proc = finalize_init(proc, tile)
            if ~isempty(proc.PARA.start_time_slice) && sum(isnan(proc.PARA.start_time_slice)) == 0
                ...
                % if size(tile.FORCING.PARA.start_time,1) == 3
                %     proc.PARA.start_time_slice = datenum(tile.FORCING.PARA.start_time(1), tile.FORCING.PARA.start_time(2), tile.FORCING.PARA.start_time(3));
                % else
            ...
            if ~isempty(proc.PARA.end_time_slice) && sum(isnan(proc.PARA.end_time_slice)) == 0
                ...
                % if size(tile.FORCING.PARA.end_time,1) == 3
                %     proc.PARA.end_time_slice = datenum(tile.FORCING.PARA.end_time(1), tile.FORCING.PARA.end_time(2), tile.FORCING.PARA.end_time(3));
                % else
