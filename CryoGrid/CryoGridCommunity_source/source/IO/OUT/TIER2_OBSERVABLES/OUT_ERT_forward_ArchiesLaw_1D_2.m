...
classdef OUT_ERT_forward_ArchiesLaw_1D_2 < OUT_BASE_OBSERVABLES
    ...
    methods
        ...
        function out = state2out(out,tile)
            ...
            conductivity_surface = (1+temperature_dependence_coefficient.*(T-T0)) .* porosity.^(out.PARA.cementation_exponent-1) .* 10.^(out.PARA.surface_conductivity);
            ...
            if out.PARA.logarithmic
                out.STATVAR.potentials = [out.STATVAR.potentials; log(result')];
            else
