...
classdef CLUSTER_SLOPE_ASPECT < matlab.mixin.Copyable
    ...
    methods
        ...
        function data = provide_data(cluster)
         ...
         data = [-cosd(cluster.SPATIAL.STATVAR.aspect).*sind(cluster.SPATIAL.STATVAR.slope_angle) sind(cluster.SPATIAL.STATVAR.aspect).*sind(cluster.SPATIAL.STATVAR.slope_angle)];
        end
