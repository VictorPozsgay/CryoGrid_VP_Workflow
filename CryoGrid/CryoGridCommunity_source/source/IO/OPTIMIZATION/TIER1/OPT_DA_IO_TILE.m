%IO functions for DA for normal TILEs with parallel computing, including communication between
%workers


classdef OPT_DA_IO_TILE < matlab.mixin.Copyable
                ...
                if iscell(tile.OUT)
                    for j=1:size(tile.OUT,1)
                        if strcmp(da.PARA.observable_classes{i,1}, class(tile.OUT{j,1})) && da.PARA.observable_classes_index(i,1) == tile.OUT{j,1}.PARA.class_index
                            %modeled_obs = [modeled_obs; move_out2obs(tile.OUT{j,1})]; %make this a column vector, as in collect_observations()
                            a = move_out2obs(tile.OUT{j,1});
                            a = a';
                            modeled_obs = [modeled_obs; a(:)];
                        end
                    end
                else
                    %modeled_obs = [modeled_obs; move_out2obs(tile.OUT)]; %make this a column vector, as in collect_observations()
                    a = move_out2obs(tile.OUT);
                    a = a';
                    modeled_obs = [modeled_obs; a(:)];
                end
            end
