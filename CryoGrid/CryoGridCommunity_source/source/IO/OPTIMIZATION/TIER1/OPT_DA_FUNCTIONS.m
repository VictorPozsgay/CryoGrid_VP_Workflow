classdef OPT_DA_FUNCTIONS < matlab.mixin.Copyable
    ...
    methods
        ...
        function da = load_observations(da, run_info)
            ...
            for i=1:size(da.PARA.observation_classes,1)
                ...
                da.OBSERVATIONS{i,1} = copy(run_info.PPROVIDER.CLASSES.(da.PARA.observation_classes{i,1}){da.PARA.observation_classes_index(i,1)});
                da.OBSERVATIONS{i,1}  = finalize_init(da.OBSERVATIONS{i,1}, run_info);
                da.OBSERVATIONS{i,1}  = read_observations(da.OBSERVATIONS{i,1}, run_info);
                ...
                %da.STATVAR.modeled_obs{i,1} = repmat(da.STATVAR.observations{i,1}.*NaN,1, get_size_of_modeled_obs(da.IO, da, run_info));
                da.OBSERVATIONS{i,1}.STATVAR = [];

                %move this to OUT and later
        ...
        function da = assign_prior_state(da, run_info)
            tile_class = class(run_info.TILE);
            a = str2func(tile_class);
            realization_number = [1:da.RUN_INFO.ENSEMBLE.TEMP.ensemble_size]';
        ...
        function da = get_ensemble_info(da, run_info)
            ...
            for i=1:size(da.PARA.ensemble_variables,1)
                pos_in_ensemble = [pos_in_ensemble; find(strcmp(da.PARA.ensemble_variables{i,1}, run_info.ENSEMBLE.TEMP.variable_name))];
                value_gaussian = [value_gaussian; run_info.ENSEMBLE.STATVAR.([da.PARA.ensemble_variables{i,1} '_gaussian'])'];
            end
            da.ENSEMBLE.value_gaussian  = value_gaussian;
            da.TEMP.old_value_gaussian  = value_gaussian;
        ...
        function da = collect_observations(da, tile)
            ...
            for i=1:size(da.STATVAR.observations,1)
                range = find(da.STATVAR.obs_time{i,1} > tile.PARA.start_time & da.STATVAR.obs_time{i,1} <= tile.PARA.end_time);
                %observations = [observations; da.STATVAR.observations{i,1}(range,:)]; %make this a column vector
                a = da.STATVAR.observations{i,1}(range,:);
                a = a';
                observations = [observations; a(:)];

                %obs_variance = [obs_variance; da.STATVAR.obs_variance{i,1}(range,:)];
                a = da.STATVAR.obs_variance{i,1}(range,:);
                a = a';
                obs_variance = [obs_variance; a(:)];
            end
        ...
        function da = resample_AMIS(da, run_info)
            ...
            value_gaussian_resampled=reshape(value_gaussian_resampled,[size(value_gaussian_resampled,1), da.TEMP.ensemble_size_DA.*da.TEMP.num_iterations]); % Concatenate across all weights (past proposals)
            value_gaussian_resampled = value_gaussian_resampled(:,resample_ID);
            ...
            cov_gaussian_resampled=(1./da.TEMP.ensemble_size_DA).*(A*A'); % proposal covariance for next iteration

            d = min(2.*(da.ENSEMBLE.effective_ensemble_size-1)./da.TEMP.ensemble_size_DA./da.PARA.min_ensemble_diversity,1); %d=min(diversity/adapt_thresh,1);
            ...
            if clip>=2 %da.ENSEMBLE.effective_ensemble_size>2 %sum(da.ENSEMBLE.weights(:) > 1/(10*da.TILE.PARA.ensemble_size))>clip  %should have a threshold, is 0 in Kris original code
                ...
                resamplec_ID = randsample(da.TEMP.ensemble_size_DA.*da.TEMP.num_iterations, da.TEMP.ensemble_size_DA, true, w);
                thetapc=reshape(thetapc,[size(thetapc,1), da.TEMP.ensemble_size_DA.*da.TEMP.num_iterations]); %reshape(thetapc,[Np,Nw]);
            ...
            Z = randn(size(mean_gaussian_resampled,1), da.TEMP.ensemble_size_DA); %Z=randn(Np,Ne);
