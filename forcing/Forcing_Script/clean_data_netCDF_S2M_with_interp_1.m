...
formatOut = 'ddmmmyyyy';
ttab_outname = sprintf('SAFRAN_ttab_elev_%d_slope_%d_aspect_%d_massif_%d_dates_%s_%s', ...
    elev, slope_ind, aspect_ind, massif_num, ...
    datestr(ttab.Properties.RowTimes(1), formatOut), ...
    datestr(ttab.Properties.RowTimes(end), formatOut));

ttab_safran = ttab;
save(fullfile(table_out_folder, ttab_outname), 'ttab_safran', 'info');
